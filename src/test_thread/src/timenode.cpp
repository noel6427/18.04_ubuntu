...
//Dynamixel Control
#include <test_thread/test_threa
d.h>
...
struct End_point one(struct End_point EP_goal);
...
struct End_point number1 (End_point EP_goal){
   if EP_goal.y < 1.;

   return EP_goal;
}

struct End_point circle (End_point EP_goal){
   EP_goal.x = 0.1;
   EP_goal.y += 0.1;

   return EP_goal;
}
struct End_point L (End_point EP_goal){
   if(EP_goal.y<2.0){
   EP_goal.y+=0.1;
   }
   else if(EP_goal.x<1.0){
     EP_goal.x+=0.1;
   }
   return EP_goal;
}
struct End_point tri (End_point EP_goal){
   EP_goal.x = 0.1;
   EP_goal.y += 0.1;
